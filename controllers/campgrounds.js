...
const getCampgrounds = async (StartValue, direction = 'f') => {
    ...
    let campgrounds;
    if (direction === 'b') {
        campgrounds = await Campground.find({decrementalIndex: {$gt: StartValue}}).hint({decrementalIndex: 1}).limit(campgroundsPerPage);
        campgrounds.reverse();
        if (!campgrounds.length)
            campgrounds = await Campground.find({_id: {$gt: baseValue}}).limit(campgroundsPerPage);
        else {
            startValue = await Campground.findOne({decrementalIndex: {$gt: campgrounds[0].decrementalIndex}}).hint({decrementalIndex: 1});
            if (!startValue) startValue = baseValue;
            else startValue = startValue._id;
        }
    } else {
        campgrounds = await Campground.find({_id: {$gt: StartValue}}).limit(campgroundsPerPage);
        if (!campgrounds.length) {
            campgrounds = await Campground.find().hint({decrementalIndex: 1}).limit(numberPerPage);
            campgrounds.reverse();
        }
    }
...
module.exports.index = async (req, res) => {
    ...
    if (!result)
        result = await getCampgrounds(startValue);
    if (!result) return res.render('campgrounds/index', {campgrounds: null, authorCampgrounds: false})
    ...
    res.render('campgrounds/index', {campgrounds, verificationString, authorCampgrounds: false})
}
...
module.exports.authorsCampgrounds = async (req, res) => {
    ...
    if (!campgrounds) return res.render('campgrounds/index', {campgrounds: null})
    res.render('campgrounds/index', {campgrounds, authorCampgrounds: true})
}
